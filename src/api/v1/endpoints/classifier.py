...
from src.core.exceptions import AppBaseException, ClassificationFailedException, ValidationException
...
async def classify_intent(
    request: ClassifyIntentRequest, intent_service: IntentServiceDep
) -> ClassifyIntentResponse:
    """
    ...
    try:
        ...
        result = await intent_service.classify(user_input=request.text, request_id=request_id)
...
async def classify_intent_batch(
    request: BatchClassifyIntentRequest, intent_service: IntentServiceDep
) -> BatchClassifyIntentResponse:
    """
    ...
    try:
        ...
        results = await intent_service.classify_batch(
            user_inputs=request.texts, request_id=request_id
        )
        ...
        for idx, result in enumerate(results):
            if result.intent == IntentType.UNKNOWN and result.confidence == 0.0:
                ...
                errors.append({"index": idx, "text": request.texts[idx][:100], "error": error_info})
...
async def get_model_info(
    intent_service: IntentServiceDep, prompt_manager: PromptManagerDep
) -> ModelInfoResponse:
    """
    ...
    log_api_request(logger, method="GET", path="/api/v1/classify/model/info", request_id=request_id)
