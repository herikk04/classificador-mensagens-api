...
class ClassifyIntentRequest(BaseModel):
    ...
    include_raw_response: bool = Field(
        default=False, description="Incluir resposta bruta do LLM no resultado"
    )

    include_metadata: bool = Field(
        default=False, description="Incluir metadados adicionais no resultado"
    )
    ...
    def validate_request_id(cls, v: Optional[str]) -> Optional[str]:
        ...
        if not v.replace("_", "").replace("-", "").isalnum():
            raise ValueError("request_id deve conter apenas letras, números, hífens e underscores")
...
class BatchClassifyIntentRequest(BaseModel):
    ...
    include_raw_response: bool = Field(
        default=False, description="Incluir resposta bruta do LLM em cada resultado"
    )

    include_metadata: bool = Field(
        default=False, description="Incluir metadados adicionais em cada resultado"
    )

    fail_on_first_error: bool = Field(
        default=False, description="Interromper processamento ao encontrar primeiro erro"
    )

    model_config = {
        "json_schema_extra": {
            "example": {
                "texts": ["Bom dia!", "Como faço para cancelar?", "Muito obrigado pela ajuda"],
    ...
    def validate_texts(cls, v: list[str]) -> list[str]:
        ...
        for idx, text in enumerate(v):
            ...
            if not text:
                raise ValueError(f"Texto na posição {idx} está vazio ou contém apenas espaços")
            ...
            if len(text) > 1000:
                raise ValueError(f"Texto na posição {idx} excede o limite de 1000 caracteres")
...
class LLMConfigOverrideRequest(BaseModel):
    ...
    temperature: Optional[float] = Field(
        default=None, ge=0.0, le=2.0, description="Temperatura para geração (override do padrão)"
    )

    max_tokens: Optional[int] = Field(
        default=None, ge=1, le=8192, description="Máximo de tokens (override do padrão)"
    )

    max_examples: Optional[int] = Field(
        default=None, ge=1, le=20, description="Número de exemplos few-shot (override do padrão)"
    )

    model_config = {
        "json_schema_extra": {"example": {"temperature": 0.5, "max_tokens": 256, "max_examples": 3}}
    }
...
class ClassifyIntentWithConfigRequest(ClassifyIntentRequest):
    ...
    config_override: Optional[LLMConfigOverrideRequest] = Field(
        default=None, description="Configurações customizadas para esta requisição"
    )
    ...
    model_config = {
        "json_schema_extra": {
            "example": {
                ...
                "config_override": {"temperature": 0.1, "max_tokens": 128, "max_examples": 3},
            }
        }
    }
