...
class GeminiClient(ILLMProvider):
        ...
        **kwargs: Any
        ...
        try:
            ...
            response = await asyncio.wait_for(
                self._async_generate_content(model, prompt),
        ...
        try:
            ...
            if not response.candidates:
                raise GeminiInvalidResponseException(
                    reason="Resposta sem candidates",
            ...
            if not text:
                raise GeminiInvalidResponseException(
                    reason="Texto vazio na resposta",
                    response_text=str(response)
                )
            ...
            raise GeminiInvalidResponseException(
                reason=f"Estrutura de resposta inválida: {str(e)}",
                response_text=str(response)
            ) from e
    ...
    async def health_check(self) -> bool:
        ...
        try:
            ...
            response = await asyncio.wait_for(
                self._async_generate_content(self._model, test_prompt),
                timeout=10
            )
