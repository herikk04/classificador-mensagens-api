...
class PromptManager(IPromptManager):
    ...
    def __init__(
        self, examples_file_path: Optional[str] = None, max_examples: Optional[int] = None
    ) -> None:
        """
        ...
        try:
            ...
            if not file_path.exists():
                raise ExamplesLoadException(
                    file_path=str(file_path), reason="Arquivo não encontrado"
                )

            if not file_path.is_file():
                raise ExamplesLoadException(
                    file_path=str(file_path), reason="Path não é um arquivo válido"
                )
            ...
            if not isinstance(examples_data, list):
                raise ExamplesLoadException(
                    file_path=str(file_path), reason="Campo 'examples' deve ser uma lista"
                )
            ...
            for idx, item in enumerate(examples_data):
                try:
                    ...
                    logger.warning(f"Exemplo {idx} inválido e será ignorado: {str(e)}")

            if not examples:
                raise ExamplesLoadException(
                    file_path=str(file_path), reason="Nenhum exemplo válido encontrado"
                )
            ...
            raise ExamplesLoadException(
                file_path=str(self.examples_file_path), reason=f"Erro ao decodificar JSON: {str(e)}"
            ) from e
            ...
            raise ExamplesLoadException(
                file_path=str(self.examples_file_path), reason=f"Erro inesperado: {str(e)}"
            ) from e
    ...
    async def build_prompt(
        ...
        try:
            ...
            # Constrói o prompt
            prompt_parts = [self.get_system_instruction(), "", "## Exemplos de Classificação:", ""]
            ...
            prompt_parts.extend(["## Tarefa:", f"Input: {user_input}", "Output:"])
