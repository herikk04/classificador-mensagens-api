...
class IntentService(IIntentClassifier):
    ...
    def __init__(self, llm_provider: ILLMProvider, prompt_manager: IPromptManager) -> None:
        """
        ...
        logger.info(f"IntentService inicializado: model={self.llm_provider.get_model_name()}")

    async def classify(
        self, user_input: str, request_id: Optional[str] = None
        ...
        try:
            ...
            processing_time_ms = (time.time() - start_time) * 1000
            logger.error(
                f"Erro na classificação: {str(e)}, time={processing_time_ms:.2f}ms", exc_info=True
            )
    ...
    async def classify_batch(
        self, user_inputs: list[str], request_id: Optional[str] = None
        ...
        for idx, user_input in enumerate(user_inputs):
            try:
                ...
                errors.append({"index": idx, "input": user_input[:100], "error": str(e)})
                ...
                results.append(
                    ClassificationResult(
                        ...
                        model_used=self.llm_provider.get_model_name(),
                        metadata={"request_id": sub_request_id, "error": str(e)},
                    )
                )
    ...
    def _validate_input(self, user_input: str) -> None:
        ...
        if not user_input or not user_input.strip():
            raise ValidationException(message="O texto não pode estar vazio", field="user_input")
