...
class ILLMProvider(ABC):
    ...
    async def generate_completion(
        self, prompt: str, temperature: float = 0.3, max_tokens: int = 512, **kwargs: Any
    ) -> str:
        """
...
class IPromptManager(ABC):
    ...
    async def build_prompt(
        self, user_input: str, examples: list[FewShotExample] | None = None, max_examples: int = 5
    ) -> str:
        """
...
class IIntentClassifier(ABC):
    ...
    @abstractmethod
    async def classify(
        self, user_input: str, request_id: str | None = None
    ...
    async def classify_batch(
        self, user_inputs: list[str], request_id: str | None = None
